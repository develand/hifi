        ...
        describe('Look & Feel', () => {
            describe('Interaction with AccountsList', () => {
                ...
                it('the overview tab lists all selected accounts in order by account number', () => {
                    ...
                    let firstCol = firstDataRow.find(Col);
                    expect(firstCol.find('div').at(0).props.children).to.contain(FIRST_ACCOUNT_NUMBER);
                    const secondDataRow = gridRows.at(2);
                    firstCol = secondDataRow.find(Col);
                    expect(firstCol.find('div').at(0).props.children).to.contain(SECOND_ACCOUNT_NUMBER);
                });
            });
        });
    });
