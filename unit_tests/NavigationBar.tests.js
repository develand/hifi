  ...
  describe('Navbar', () => {
    describe('Look & Feel', function() {
      ...
      it('has a "Future Look" button', () => {
        expect(navbarElem.find(NavItem).at(0).text()).to.equal('Future Look');
      });
      it('has a "Past Performance" button', () => {
        expect(navbarElem.find(NavItem).at(1).text()).to.equal('Past Performance');
      });
      it('has a "Portfolio Breakdown" button', () => {
        expect(navbarElem.find(NavItem).at(2).text()).to.equal('Portfolio Breakdown');
      });
      it('has a "Load Test Data" button', () => {
        expect(navbarElem.find(NavItem).at(3).text()).to.equal('Load Test Data');
      });
    });
  });
}
